...
import { wordHref, spellingHref, translitToUnicode } from "@/lib/word-utils";
...
export function WordCardList({ words, max }: WordCardListProps) {
  ...
  return (
    <div className="space-y-2">
      {items.map((word) => (
        <Link
          key={`${word.transliteration}_${word.mdc}`}
          href={word.gardinerCodes?.length ? spellingHref(word.gardinerCodes) : wordHref(word.transliteration)}

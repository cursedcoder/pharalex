...
TLA Lemmata CSV           → scripts/integrate-tla.ts       ─→ (merges into words.json: new entries + gender/TLA/Wikidata IDs)
...
TLA corpora + PhilologEg  → scripts/build-attestations.ts  ─→ words.json (attestation counts + sense ordering)
